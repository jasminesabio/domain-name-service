...
contract Domains is ERC721URIStorage {
    ...
    function register(string calldata name) public payable {
        ...
        console.log("Registering %s.%s on the contract wiht tokenID #s", name, tld, newRecordId);
    ...
    function setRecord(string calldata name, string calldata record) public {
        ...
        if (msg.sender != domains[name]) revert Unauthorzied();
        records[name] = record;
    }
    ...
    function getAllNames() public view returns (string[] memory) {
        console.log("Getting all naems from contract.");
        string[] memory allNames = new string[](_tokenIds.current());
        for (uint i = 0; i < _tokenIds.current(); i++) {
            allNames[i] = names[i];
            console.log("Name for token %s is %s", i, allNames[i]);
        }
